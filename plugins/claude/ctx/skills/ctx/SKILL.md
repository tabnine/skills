...
- **[`ctx-onboarding`](../ctx-onboarding/SKILL.md)** — check the agent model + embedder are configured (guide the operator to set them in the UI if not), connect data sources (repos + credentials) and trigger sync, then tour what it knows (stats, graph, capability map). Start here to onboard a tenant.
...
| **Onboard a tenant / get started / connect repos / "what can it do for me"** | _check model+embedder · connect sources · stats_ | — | [`ctx-onboarding`](../ctx-onboarding/SKILL.md) |
