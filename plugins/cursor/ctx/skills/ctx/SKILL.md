...
- **[`ctx-onboarding`](../ctx-onboarding/SKILL.md)** — check the agent model + embedder are configured (operator sets them in the UI), connect data sources (repos + credentials) and ingest, then immediately show what CTX understood about each repo (what it is / stack / does / talks-to + a live semantic-search hit) and invite questions — the slower service/dependency-graph enrichment agents run as a follow-on. Start here to onboard a tenant.
...
| **Onboard a tenant / connect repos / "what does CTX know about my system"** | _check model+embedder · connect & ingest · show what CTX understood (fast) · ask questions · enrichment agents follow on_ | — | [`ctx-onboarding`](../ctx-onboarding/SKILL.md) |
